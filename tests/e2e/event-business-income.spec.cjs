/**
 * BUSINESS_INCOME Event Tests
 * Tests the BUSINESS_INCOME event type for monthly business income scenarios
 */
...
test('monthly business income event', async ({ page }) => {
  console.log('Testing BUSINESS_INCOME event: Monthly business income');
  
  const businessIncomeCase = {
    initialAccounts: { 
      cash: 8000  // Start with some cash, business income will add to it
    },
    events: [
      {
        id: 'monthly-business',
    ...
    monthsToRun: 3, // Test 3 months of business income
  ...
  await jsonEditor.fill(JSON.stringify(businessIncomeCase, null, 2));
  
  console.log('Business income test case loaded');
  
  // Click run button
  const runButton = page.getByRole('button', { name: '🚀 Run Simulation from JSON' });
  await runButton.click();
  
  console.log('Business income simulation started...');
  ...
  console.log('Business income simulation results:', {
    ...
    totalBusinessIncomeExpected: 4500 * 3, // 3 months at $4500/month
  ...
  // Final net worth should be initial cash plus total business income plus modest returns
  expect(results.finalNetWorth).toBeGreaterThan(8000); // Should be higher than initial due to income
  expect(results.finalNetWorth).toBeGreaterThan(20000); // Should be around $21.5k ($8k + $13.5k + returns)
  
  // Validate income was applied each month
  expect(results.monthlyData[0].incomeThisMonth).toBe(4500);
  expect(results.monthlyData[1].incomeThisMonth).toBe(4500);
  expect(results.monthlyData[2].incomeThisMonth).toBe(4500);
  ...
  // Final cash should reflect all business income plus returns
  const finalCash = results.monthlyData[2].accounts.cash;
  expect(finalCash).toBeGreaterThan(20000); // Approximately $8k initial + $13.5k income + returns
  expect(finalCash).toBeLessThan(23000); // But not too much more (allowing for returns)
  ...
  console.log('✅ BUSINESS_INCOME event test passed - business income working correctly');
});