/**
 * Phase 4 Task 4.2: Comprehensive Test Harness Runner (CommonJS)
 * 
...
async function discoverTestCases() {
  ...
  try {
    ...
    for (const fileName of testCaseFiles) {
      const testCasePath = path.join(TEST_CASES_DIR, fileName);
      ...
      let content;
      try {
        const fileContent = await fs.readFile(testCasePath, 'utf-8');
 ...
 */
async function waitForTestHarnessReady(page) {
  // Wait for the main heading to appear
  await page.waitForSelector('h1', { state: 'visible', timeout: 30000 });
  
  // Wait for test case dropdown to be populated - be more lenient with timeout
  await page.waitForSelector('select option[value]:not([value=""])', { timeout: 20000 });
 ...
 */
async function loadTestCase(page, testCase) {
 ...
 */
async function compareWithSnapshot(testCase, actualOutput) {
  // Read the golden snapshot
  const snapshotContent = await fs.readFile(testCase.snapshotPath, 'utf-8');
...
test.describe('PathFinder Pro Test Harness - Comprehensive Regression Testing', () => {
  ...
  test.describe('Test Harness Validation', () => {
    test('should load test harness correctly', async ({ page }) => {
      ...
      expect(count).toBeGreaterThanOrEqual(2); // Input and output areas
    });
