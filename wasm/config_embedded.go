...
func LoadEmbeddedFinancialConfig() error {
    ...
    if err := readJSON("tax_brackets_2024.json", &tb2024); err != nil { return err }

    var tb2025 TaxBracketsConfig
    if err := readJSON("tax_brackets_2025.json", &tb2025); err != nil { return err }
    ...
    if err := readJSON("rmd_table_2024.json", &rmd); err != nil { return err }
    rmdTableConfig = &rmd

    var contrib ContributionLimitsConfig
    if err := readJSON("contribution_limits_2025.json", &contrib); err != nil { return err }
    contributionConfig = &contrib

    var fica FICATaxConfig
    if err := readJSON("fica_tax_2024.json", &fica); err != nil { return err }
    ficaTaxConfig = &fica

    var st StateTaxBracketsConfig
    if err := readJSON("state_tax_brackets.json", &st); err != nil { return err }
    stateTaxConfig = &st

    var ir IRMAABracketsConfig
    if err := readJSON("irmaa_brackets_2024.json", &ir); err != nil { return err }
    irmaaConfig = &ir

    var ar AssetReturnsConfig
    if err := readJSON("asset_returns_historical.json", &ar); err != nil { return err }
    assetReturnsConfig = &ar

    var mre MonthlyRealEstateConfig
    if err := readJSON("monthly_real_estate_data.json", &mre); err != nil { return err }
    monthlyRealEstateConfig = &mre

    var div DividendModelConfig
    if err := readJSON("dividend_model_data.json", &div); err != nil { return err }
    dividendModelConfig = &div

    var def DefaultsConfig
    if err := readJSON("defaults.json", &def); err != nil { return err }
    defaultsConfig = &def
