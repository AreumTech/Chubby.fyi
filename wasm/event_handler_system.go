...
func (h *SystemEventHandlerSimple) processCashShortfallCheck(accounts *AccountHoldingsMonthEnd, monthOffset int) error {
	...
	if accounts.Cash < 0 {
		netAmountNeeded := -accounts.Cash + 100 // Net amount to get back to at least $100 positive

		// Gross up withdrawal to account for tax withholding
		// This ensures we end up with the net amount needed after taxes are deducted
		grossAmount := h.engine.grossUpForTaxes(netAmountNeeded, accounts)

		simLogVerbose("🚨 [NEGATIVE_CASH_BACKUP] Month %d: Cash=$%.2f is negative, need net $%.2f, withdrawing gross $%.2f",
			monthOffset, accounts.Cash, netAmountNeeded, grossAmount)

		// Use investments-only withdrawal (don't try to use cash, since it's already negative)
		saleResult := h.engine.cashManager.SellFromInvestmentsOnly(accounts, grossAmount, monthOffset)
		...
		if taxWithheld > 0 {
			simLogVerbose("💸 [NEGATIVE_CASH_BACKUP] Tax withheld on withdrawal: $%.2f", taxWithheld)
		}
		...
		simLogVerbose("🚨 [CASH_SHORTFALL] Cash=$%.2f below minimum buffer=$%.2f, need net $%.2f, withdrawing gross $%.2f",
			accounts.Cash, minCashBuffer, netAmountNeeded, grossAmount)

		// Use investments-only withdrawal (already accounted for existing cash in shortfall calculation)
		saleResult := h.engine.cashManager.SellFromInvestmentsOnly(accounts, grossAmount, monthOffset)
		...
		taxWithheld := h.engine.ApplyImmediateWithdrawalTax(saleResult, accounts)
		if taxWithheld > 0 {
			simLogVerbose("💸 [FORCED_WITHDRAWAL] Tax withheld on withdrawal: $%.2f", taxWithheld)
		}
		...
		simLogVerbose("💸 [FORCED_WITHDRAWAL] Month %d: Withdrew $%.2f to raise cash from $%.2f to $%.2f (target: $%.2f). DivestmentProceeds: $%.2f",
			monthOffset, saleResult.TotalProceeds,
			accounts.Cash-saleResult.TotalProceeds+taxWithheld, // Cash before withdrawal
