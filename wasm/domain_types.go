...
// SimpleTaxConfig represents simplified tax configuration for MCP Bronze tier
// Uses effective rates instead of full bracket-based calculations
type SimpleTaxConfig struct {
	Enabled          bool    `json:"enabled"`          // Whether taxes are applied
	EffectiveRate    float64 `json:"effectiveRate"`    // Effective ordinary income tax rate (0-1), e.g., 0.22 for 22%
	CapitalGainsRate float64 `json:"capitalGainsRate"` // Long-term capital gains rate (0-1), e.g., 0.15 for 15%
	FilingStatus     string  `json:"filingStatus"`     // "single" or "married" (informational)
	State            string  `json:"state"`            // State code or "NONE" for federal-only
}
...
type SimulationInput struct {
	...
	TaxConfig          *SimpleTaxConfig        `json:"taxConfig,omitempty"`        // Simplified tax config for Bronze tier
}
...
const (
	...
	WithdrawalSequenceTaxDeferredFirst WithdrawalSequence = "TAX_DEFERRED_FIRST" // Cash -> Tax-deferred -> Taxable -> Roth
	...
	WithdrawalSequenceProportional     WithdrawalSequence = "PROPORTIONAL"       // Proportional from all accounts (maintains allocation)
	WithdrawalSequenceRothFirst        WithdrawalSequence = "ROTH_FIRST"         // Roth -> Taxable -> Tax-Deferred -> Cash
)
...
type LotSaleResult struct {
	...
	// Account type breakdown for tax withholding
	TaxDeferredProceeds float64 `json:"taxDeferredProceeds"` // Amount from tax-deferred (taxed as ordinary income)
	TaxableProceeds     float64 `json:"taxableProceeds"`     // Amount from taxable (only gains taxed)
	RothProceeds        float64 `json:"rothProceeds"`        // Amount from Roth (tax-free)
	CashUsed            float64 `json:"cashUsed"`            // Amount from cash (no tax)
}
...
type AssetAllocationStrategy struct {
	...
	TargetRetirementAge int                    `json:"targetRetirementAge"` // For glide_path: target retirement age (default 65)
}
