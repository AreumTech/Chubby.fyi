...
func (h *RecurringExpenseEventHandler) Process(event FinancialEvent, accounts *AccountHoldingsMonthEnd, cashFlow *float64, context *EventProcessingContext) error {
	...
	if accounts.Cash < expenseAmount {
		netShortfall := expenseAmount - accounts.Cash

		// Gross up the withdrawal to account for tax withholding
		// This ensures we have enough cash after taxes to cover the expense
		grossAmount := se.grossUpForTaxes(netShortfall, accounts)

		simLogVerbose("💰 [PRE-EXPENSE-CHECK] Month %d: Insufficient cash ($%.2f) for expense ($%.2f), need net $%.2f, withdrawing gross $%.2f",
			context.CurrentMonth, accounts.Cash, expenseAmount, netShortfall, grossAmount)

		// Attempt to raise cash from investments ONLY (don't use existing cash)
		// We already accounted for existing cash in the shortfall calculation
		saleResult := se.cashManager.SellFromInvestmentsOnly(accounts, grossAmount, context.CurrentMonth)
		...
		if taxWithheld > 0 {
			simLogVerbose("💸 [PRE-EXPENSE-CHECK] Tax withheld on withdrawal: $%.2f", taxWithheld)
		}
...
func (h *OneTimeExpenseEventHandler) Process(event FinancialEvent, accounts *AccountHoldingsMonthEnd, cashFlow *float64, context *EventProcessingContext) error {
	...
	simLogVerbose("💰 [ONE_TIME_EXPENSE] Month %d: Event=%s, Amount=$%.2f, CashBefore=$%.2f",
		context.CurrentMonth, event.ID, expenseAmount, accounts.Cash)
	...
	if accounts.Cash < expenseAmount {
		...
		simLogVerbose("💰 [ONE_TIME_EXPENSE-CHECK] Month %d: Insufficient cash ($%.2f) for expense ($%.2f), need net $%.2f, withdrawing gross $%.2f",
			context.CurrentMonth, accounts.Cash, expenseAmount, netShortfall, grossAmount)
		...
		taxWithheld := se.ApplyImmediateWithdrawalTax(saleResult, accounts)
		if taxWithheld > 0 {
			simLogVerbose("💸 [ONE_TIME_EXPENSE-CHECK] Tax withheld on withdrawal: $%.2f", taxWithheld)
		}
		...
		simLogVerbose("💰 [ONE_TIME_EXPENSE-CHECK] Raised $%.2f from investments, cash now $%.2f",
			saleResult.TotalProceeds, accounts.Cash)
	}

	// Now process the expense
	accounts.Cash -= expenseAmount
	*cashFlow -= expenseAmount
