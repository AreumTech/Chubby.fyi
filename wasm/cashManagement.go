...
func (cm *CashManager) ExecuteWithdrawalWithStrategy(accounts *AccountHoldingsMonthEnd, targetAmount float64, strategy WithdrawalSequence, currentMonth int, ytdOrdinaryIncome float64, filingStatus FilingStatus) (LotSaleResult, float64) {
	simLogVerbose("WITHDRAWAL-STRATEGY: Using strategy %s for $%.0f", strategy, targetAmount)
	switch strategy {
	...
	case WithdrawalSequenceProportional:
		return cm.executeProportionalWithdrawal(accounts, targetAmount, currentMonth)
	case WithdrawalSequenceRothFirst:
		return cm.executeRothFirstWithdrawal(accounts, targetAmount, currentMonth)
	default:
		simLogVerbose("WITHDRAWAL-STRATEGY: Unknown strategy '%s', defaulting to TAX_EFFICIENT", strategy)
		return cm.executeConventionalWithdrawal(accounts, targetAmount, currentMonth)
	}
}

// SellFromInvestmentsOnly sells from investment accounts ONLY (skipping cash).
// Used when the caller has already accounted for existing cash and needs to raise
// additional funds from investments. Order: Taxable -> Tax-Deferred -> Roth
func (cm *CashManager) SellFromInvestmentsOnly(accounts *AccountHoldingsMonthEnd, targetAmount float64, currentMonth int) LotSaleResult {
	simLogVerbose("INVESTMENTS-ONLY-WITHDRAWAL ENTRY: Target $%.0f", targetAmount)
	...
	// Step 1: Sell from taxable account
	if taxableAccount := GetTaxableAccount(accounts); remainingNeeded > 0 && taxableAccount != nil && taxableAccount.TotalValue > 0 {
		simLogVerbose("INVESTMENTS-ONLY Step 1: Selling from taxable, value $%.0f", taxableAccount.TotalValue)
		taxableResult := cm.SellAssetsFromAccountFIFO(taxableAccount, remainingNeeded, currentMonth)
		totalResult.TaxableProceeds += taxableResult.TotalProceeds
		cm.mergeSaleResults(&totalResult, taxableResult) // TotalProceeds added here
		remainingNeeded -= taxableResult.TotalProceeds
		simLogVerbose("INVESTMENTS-ONLY Step 1: Sold $%.0f from taxable, remaining $%.0f", taxableResult.TotalProceeds, remainingNeeded)
	}

	// Step 2: Sell from tax-deferred account
	if taxDeferredAccount := GetTaxDeferredAccount(accounts); remainingNeeded > 0 && taxDeferredAccount != nil && taxDeferredAccount.TotalValue > 0 {
		simLogVerbose("INVESTMENTS-ONLY Step 2: Selling from tax-deferred, value $%.0f", taxDeferredAccount.TotalValue)
		taxDeferredResult := cm.SellAssetsFromAccountFIFO(taxDeferredAccount, remainingNeeded, currentMonth)
		totalResult.TaxDeferredProceeds += taxDeferredResult.TotalProceeds
		cm.mergeSaleResults(&totalResult, taxDeferredResult) // TotalProceeds added here
		remainingNeeded -= taxDeferredResult.TotalProceeds
		simLogVerbose("INVESTMENTS-ONLY Step 2: Sold $%.0f from tax-deferred, remaining $%.0f", taxDeferredResult.TotalProceeds, remainingNeeded)
	}

	// Step 3: Sell from Roth account (last resort)
	if rothAccount := GetRothAccount(accounts); remainingNeeded > 0 && rothAccount != nil && rothAccount.TotalValue > 0 {
		simLogVerbose("INVESTMENTS-ONLY Step 3: Selling from Roth, value $%.0f", rothAccount.TotalValue)
		rothResult := cm.SellAssetsFromAccountFIFO(rothAccount, remainingNeeded, currentMonth)
		totalResult.RothProceeds += rothResult.TotalProceeds
		cm.mergeSaleResults(&totalResult, rothResult) // TotalProceeds added here
		remainingNeeded -= rothResult.TotalProceeds
		simLogVerbose("INVESTMENTS-ONLY Step 3: Sold $%.0f from Roth, remaining $%.0f", rothResult.TotalProceeds, remainingNeeded)
	}

	simLogVerbose("INVESTMENTS-ONLY FINAL: TotalProceeds $%.0f (taxDeferred=$%.0f, taxable=$%.0f, roth=$%.0f)",
		totalResult.TotalProceeds, totalResult.TaxDeferredProceeds, totalResult.TaxableProceeds, totalResult.RothProceeds)
	return totalResult
}

// executeConventionalWithdrawal implements "Conventional" strategy: Cash -> Taxable -> Tax-Deferred -> Roth
func (cm *CashManager) executeConventionalWithdrawal(accounts *AccountHoldingsMonthEnd, targetAmount float64, currentMonth int) (LotSaleResult, float64) {
	...
	// Step 3: Withdraw from tax-deferred account (full amount taxed as ordinary income)
	if taxDeferredAccount := GetTaxDeferredAccount(accounts); remainingNeeded > 0 && taxDeferredAccount != nil && taxDeferredAccount.TotalValue > 0 {
	...
	totalRaised := totalResult.TotalProceeds + totalResult.CashUsed
	simLogVerbose("CONVENTIONAL-WITHDRAWAL FINAL: TotalProceeds $%.0f, cashUsed $%.0f, totalRaised $%.0f (taxDeferred=$%.0f, taxable=$%.0f, roth=$%.0f)",
		totalResult.TotalProceeds, totalResult.CashUsed, totalRaised, totalResult.TaxDeferredProceeds, totalResult.TaxableProceeds, totalResult.RothProceeds)
...
// executeProportionalWithdrawal implements proportional withdrawal from all accounts
// Maintains allocation by withdrawing proportionally based on account values
func (cm *CashManager) executeProportionalWithdrawal(accounts *AccountHoldingsMonthEnd, targetAmount float64, currentMonth int) (LotSaleResult, float64) {
	simLogVerbose("PROPORTIONAL-WITHDRAWAL ENTRY: Target $%.0f", targetAmount)
	...
	if totalValue <= 0 {
		simLogVerbose("PROPORTIONAL-WITHDRAWAL: No assets available")
		return totalResult, 0
	}

	// Withdraw proportionally from each account based on its share of total
	// Step 1: Cash (no tax)
	if accounts.Cash > 0 {
		proportion := accounts.Cash / totalValue
		cashContribution := math.Min(remainingNeeded*proportion, accounts.Cash)
		accounts.Cash -= cashContribution
		totalResult.CashUsed += cashContribution
		remainingNeeded -= cashContribution
		simLogVerbose("PROPORTIONAL-WITHDRAWAL: Cash contribution $%.0f (%.1f%%)", cashContribution, proportion*100)
	}

	// Step 2: Taxable (capital gains tax on gains only)
	if remainingNeeded > 0 && taxableAccount != nil && taxableAccount.TotalValue > 0 {
		proportion := taxableAccount.TotalValue / totalValue
		withdrawAmount := math.Min(remainingNeeded*proportion/(1-proportion), taxableAccount.TotalValue)
		if withdrawAmount > 0 {
			taxableResult := cm.SellAssetsFromAccountFIFO(taxableAccount, withdrawAmount, currentMonth)
			totalResult.TaxableProceeds += taxableResult.TotalProceeds
			cm.mergeSaleResults(&totalResult, taxableResult)
			remainingNeeded -= taxableResult.TotalProceeds
			simLogVerbose("PROPORTIONAL-WITHDRAWAL: Taxable contribution $%.0f", taxableResult.TotalProceeds)
		}
	}

	// Step 3: Tax-Deferred (full amount taxed as ordinary income)
	if remainingNeeded > 0 && taxDeferredAccount != nil && taxDeferredAccount.TotalValue > 0 {
		proportion := taxDeferredAccount.TotalValue / totalValue
		withdrawAmount := math.Min(remainingNeeded*proportion/(1-proportion), taxDeferredAccount.TotalValue)
		if withdrawAmount > 0 {
			taxDeferredResult := cm.SellAssetsFromAccountFIFO(taxDeferredAccount, withdrawAmount, currentMonth)
			totalResult.TaxDeferredProceeds += taxDeferredResult.TotalProceeds
			cm.mergeSaleResults(&totalResult, taxDeferredResult)
			remainingNeeded -= taxDeferredResult.TotalProceeds
			simLogVerbose("PROPORTIONAL-WITHDRAWAL: Tax-Deferred contribution $%.0f", taxDeferredResult.TotalProceeds)
		}
	}

	// Step 4: Roth (tax-free)
	if remainingNeeded > 0 && rothAccount != nil && rothAccount.TotalValue > 0 {
		rothResult := cm.SellAssetsFromAccountFIFO(rothAccount, remainingNeeded, currentMonth)
		totalResult.RothProceeds += rothResult.TotalProceeds
		cm.mergeSaleResults(&totalResult, rothResult)
		remainingNeeded -= rothResult.TotalProceeds
		simLogVerbose("PROPORTIONAL-WITHDRAWAL: Roth contribution $%.0f", rothResult.TotalProceeds)
	}

	totalRaised := totalResult.TotalProceeds + totalResult.CashUsed
	simLogVerbose("PROPORTIONAL-WITHDRAWAL FINAL: Total raised $%.0f (taxDeferred=$%.0f, taxable=$%.0f, roth=$%.0f, cash=$%.0f)",
		totalRaised, totalResult.TaxDeferredProceeds, totalResult.TaxableProceeds, totalResult.RothProceeds, totalResult.CashUsed)
	return totalResult, totalRaised
}

// executeRothFirstWithdrawal implements Roth-first withdrawal strategy
// Order: Roth -> Taxable -> Tax-Deferred -> Cash (preserves tax-deferred growth)
func (cm *CashManager) executeRothFirstWithdrawal(accounts *AccountHoldingsMonthEnd, targetAmount float64, currentMonth int) (LotSaleResult, float64) {
	simLogVerbose("ROTH-FIRST-WITHDRAWAL ENTRY: Target $%.0f", targetAmount)
	...
	// Step 1: Withdraw from Roth account first (tax-free but using up Roth space)
	if rothAccount := GetRothAccount(accounts); remainingNeeded > 0 && rothAccount != nil && rothAccount.TotalValue > 0 {
		simLogVerbose("ROTH-FIRST-WITHDRAWAL Step 1: Selling from Roth account, value $%.0f", rothAccount.TotalValue)
		rothResult := cm.SellAssetsFromAccountFIFO(rothAccount, remainingNeeded, currentMonth)
		totalResult.RothProceeds += rothResult.TotalProceeds
		cm.mergeSaleResults(&totalResult, rothResult)
		remainingNeeded -= rothResult.TotalProceeds
		simLogVerbose("ROTH-FIRST-WITHDRAWAL Step 1: Sold $%.0f from Roth, remaining $%.0f", rothResult.TotalProceeds, remainingNeeded)
	}

	// Step 2: Sell from taxable account (capital gains tax on gains only)
	if taxableAccount := GetTaxableAccount(accounts); remainingNeeded > 0 && taxableAccount != nil && taxableAccount.TotalValue > 0 {
		simLogVerbose("ROTH-FIRST-WITHDRAWAL Step 2: Selling from taxable account, value $%.0f", taxableAccount.TotalValue)
		taxableResult := cm.SellAssetsFromAccountFIFO(taxableAccount, remainingNeeded, currentMonth)
		totalResult.TaxableProceeds += taxableResult.TotalProceeds
		cm.mergeSaleResults(&totalResult, taxableResult)
		remainingNeeded -= taxableResult.TotalProceeds
		simLogVerbose("ROTH-FIRST-WITHDRAWAL Step 2: Sold $%.0f from taxable, remaining $%.0f", taxableResult.TotalProceeds, remainingNeeded)
	}

	// Step 3: Withdraw from tax-deferred account (full amount taxed as ordinary income)
	if taxDeferredAccount := GetTaxDeferredAccount(accounts); remainingNeeded > 0 && taxDeferredAccount != nil && taxDeferredAccount.TotalValue > 0 {
		simLogVerbose("ROTH-FIRST-WITHDRAWAL Step 3: Selling from tax-deferred account, value $%.0f", taxDeferredAccount.TotalValue)
		taxDeferredResult := cm.SellAssetsFromAccountFIFO(taxDeferredAccount, remainingNeeded, currentMonth)
		totalResult.TaxDeferredProceeds += taxDeferredResult.TotalProceeds
		cm.mergeSaleResults(&totalResult, taxDeferredResult)
		remainingNeeded -= taxDeferredResult.TotalProceeds
		simLogVerbose("ROTH-FIRST-WITHDRAWAL Step 3: Sold $%.0f from tax-deferred, remaining $%.0f", taxDeferredResult.TotalProceeds, remainingNeeded)
	}

	// Step 4: Use cash last (no tax)
	if remainingNeeded > 0 && accounts.Cash > 0 {
		cashUsed := math.Min(remainingNeeded, accounts.Cash)
		accounts.Cash -= cashUsed
		totalResult.CashUsed += cashUsed
		remainingNeeded -= cashUsed
		simLogVerbose("ROTH-FIRST-WITHDRAWAL Step 4: Used $%.0f cash, remaining $%.0f", cashUsed, remainingNeeded)
	}

	totalRaised := totalResult.TotalProceeds + totalResult.CashUsed
	simLogVerbose("ROTH-FIRST-WITHDRAWAL FINAL: Total raised $%.0f (taxDeferred=$%.0f, taxable=$%.0f, roth=$%.0f, cash=$%.0f)",
		totalRaised, totalResult.TaxDeferredProceeds, totalResult.TaxableProceeds, totalResult.RothProceeds, totalResult.CashUsed)
	return totalResult, totalRaised
}
